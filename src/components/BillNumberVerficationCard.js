...
const BillNumberVerificationCard = ({ data }) => {
    const [verifyBillNumber, setVerifyBillNumber] = useState(false);
    ...
    return (
        <div className={` bg-white rounded-lg shadow-md hover:shadow-lg transition duration-300 ease-in-out p-4 m-2 flex-shrink-0 w-64  ${getBackgroundColor()}`} onClick={handleCardClick}>
            ...
            {/* Edit gatepass modal */}
            {verifyBillNumber && (
                <BillNumber gatepassData={data} onClose={() => setVerifyBillNumber(false)} />
            )}
        </div>
    );
};

export default BillNumberVerificationCard
